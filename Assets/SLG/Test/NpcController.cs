...
public class NpcController : MonoBehaviour
{
    ...
    [Header("可活动区域（中心点）")]
    public Transform areaCenterPos;

    [Header("区域半径（100x100）")]
    public float areaHalfSize = 50f;
    [Header("转向速度（度/秒）")]
    public float turnSpeed = 180f; // 每秒旋转 180°

    private string[] animationNames = new string[11] {
        "Stand",
        "Skill_Tiaozhan",
        "Skill_Huixuanzhan",
        "Attack_Stand",
    ...
    [Header("武器特效对象名")]
    public string wuqiEffectName = "wuqi";
    ...
    void PlayRandomAnimation()
    {
        ...
        // 随机挑选动画
        string clipName = animationNames[Random.Range(0, animationNames.Length)];
        // 获取目标剪辑的 state
        ...
        // 延迟到动画结束时调用下一次
        StartCoroutine(PlayNextAfter(state.length));
    }

    IEnumerator PlayNextAfter(float delay)
    {
    ...
    void Update()
    {
        ...
        if (isRunning)
        {
            ...
            {
                // 随机一个大概 90~180° 的新方向
                float newY = transform.eulerAngles.y + Random.Range(90f, 180f);
                targetRotation = Quaternion.Euler(0, newY, 0);
            }
        }
    }
