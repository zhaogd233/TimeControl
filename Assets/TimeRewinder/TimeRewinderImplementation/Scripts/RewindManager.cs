...
public class RewindManager : MonoBehaviour
{
    ...
    private readonly List<Tuple<RewindAbstract, OutOfBoundsBehaviour>> _lateAddedRewindedObjects = new();
    private List<RewindAbstract> _rewindedObjects;
    ...
    /// </summary>
    public bool IsBeingRewinded { get; private set; }
    ...
    /// <summary>
    ///     Property defining if Circular buffers should be written to, when the system is not rewinding and should normally be
    ///     tracking values.
    /// </summary>
    ...
    private void FixedUpdate()
    {
        ...
        else
        {
            ...
            for (var i = 0; i < _lateAddedRewindedObjects.Count; i++)
            {
                ...
                if (!_lateAddedRewindedObjects[i].Item1.isActiveAndEnabled &&
                    _lateAddedRewindedObjects[i].Item2 == OutOfBoundsBehaviour.DisableDestroy)
                {
                    var objectToDestroy = _lateAddedRewindedObjects[i].Item1;
                    _lateAddedRewindedObjects.Remove(_lateAddedRewindedObjects[i]);
    ...
    /// <summary>
    ///     Call this method to rewind time by specified seconds instantly without snapshot preview. Usefull for one time
    ///     instant rewinds.
    /// </summary>
    ...
    /// <summary>
    ///     Call this method if you want to start rewinding time with ability to preview snapshots. After done rewinding,
    ///     StopRewindTimeBySeconds() must be called!!!. To update snapshot preview between, call method
    ///     SetTimeSecondsInRewind().
    /// </summary>
    /// <param name="seconds">
    ///     Parameter defining how many seconds before should the rewind preview rewind to (Parameter must be
    ///     >=0)
    /// </param>
    /// <returns></returns>
    ...
    private void CheckReachingOutOfBounds(float seconds)
    {
        ...
        if (seconds < 0) Debug.LogError("Parameter in StartRewindTimeBySeconds() must have positive value!!!");
    }
    ...
    /// <param name="automaticDestroy">
    ///     When you rewind before this object is added for tracking, you can choose if object gets
    ///     disabled or is detroyed when resuming the game.
    /// </param>
    public void AddObjectForTracking(RewindAbstract objectToRewind, OutOfBoundsBehaviour outOfBoundsBehaviour)
    {
        if (!_lateAddedRewindedObjects.Any(x => x.Item1 == objectToRewind))
        {
            ...
            _lateAddedRewindedObjects.Add(
                new Tuple<RewindAbstract, OutOfBoundsBehaviour>(objectToRewind, outOfBoundsBehaviour));
        }
    }
}